...
NOTE: to make this useful over a satellite-ground path, consider factors like rain vs. altitude!
"""
import numpy as np
...
if __name__ == '__main__':
    ...
    p.add_argument('polarization',help='polarization angle 0==horiz, 90==vert, 45==circ [degrees]',type=float)
    p.add_argument('elevation',help='elevation angle above horizon [degrees]',type=float)
    p.add_argument('-v','--verbose',action='store_true')
    p = p.parse_args()
    
    if p.freqHz<=0:
        f = np.logspace(9,12,200)
        dBkm = get_rain_atten(f, p.rainrate, p.polarization, p.elevation, p.verbose)
